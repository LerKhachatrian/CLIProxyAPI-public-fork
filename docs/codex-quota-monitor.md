...
## Recoverable routing authentication failures (2026-09-11)

A generic HTTP 401 from routed traffic is not an indefinite monitor exclusion. Once its existing timed retry deadline has elapsed, the normal usage scheduler may perform a read and, only if that read returns 401 without a future Retry-After, ask the existing auth owner for one bounded recovery. Explicit invalid-grant and OAuth-expired signals, disabled accounts, untimed exclusions, future account/quota retry floors and credential-owner backoff remain authoritative. No cooldown or account state is cleared to make a read eligible.

The regression exercises the actual coordinator-to-usage-adapter-to-auth-owner path, including changed-plan observation adoption and denial cases. Reset reads still cannot initiate renewal; shared pacing, monitoring rejection floors, fixed endpoints, total deadline and credential persistence ownership are unchanged.

Verification retains the expected old-guard failure and passing corrected recovery, deadline, identity and exclusion cases. All applicable Go tests pass across the batch with only `TestXAIExecutorExecuteVideosCreate` isolated and three passing runs of that unchanged test. A single ordinary full invocation still exposes its unrelated zero-TTFT assertion, including with lower package concurrency; this is a disclosed batch-only validation limit, not an unqualified full-suite pass or an xAI fix. Installed Widget 0.43 per-card usage invocation separately produced a genuinely newer provider observation. Current provider account recovery and candidate activation require their own live evidence.

## Live usage-recovery delivery (2026-09-11)

Source `231c9260f5638b73bd726c17d52a06fe54d61c9c` is deployed under immutable source tag `ler-live-20260911-231c9260`. The canonical binary SHA-256 is `4a1f4e3fc2832426c5a3734f82a07bdf4bf6f01640f9d07907f92c6948210d4d`; the retained rollback is `8b4569ce4954cabf275b60fc83f6530e4fea4110902ff0cb44176485fab5ccb0`. Fresh disk-budget and hash/owner/health preflight passed before the announced, task-authorized cutover. Synthetic monitor/cache/restart and competing-respawner/automatic-rollback staging passed for these exact artifacts. Independent canonical health returned HTTP 200 with the candidate hash after the deployment owner exited. This supersedes the candidate-activation gate above, not its test limitations.

One explicit usage batch entered the existing shared monitor at 13:06:08 UTC. Six eligible accounts received new explicit provider checks; normal routed traffic supplied fresh passive observations for three other accounts. All nine known observations were newer than the batch request and reported Pro. The existing file-source correction had already recovered three upgraded accounts through the auth owner. Provider-reported exhaustion remains exhaustion: the refresh does not replenish usage or force an expected aggregate. Three explicit intents remain subject to their existing provider retry floors and eight-hour pending bound; they were not silently discarded or forcibly unblocked.

Installed Widget 0.43 remained independently running, reconnected without replacement, and exposed matching current account values and the partial known-balance aggregate through its actual native accessibility tree at 13:10:14 UTC. The earlier installed bulk action and the individual per-card action are separately verified; the individual action produced a newer provider observation about 605 milliseconds after invocation. No reset action was used. The final screenshot attempt produced no usable pixels, so this acceptance is native control/text evidence rather than an additional visual-layout claim.

### Remaining account-specific acceptance

One file-backed account still has no usable usage observation and retains old subscription metadata. Its bounded usage attempt at 13:06:08 UTC completed with the monitor's `oauth_rejected` result at 13:06:11 UTC. That enum does not distinguish credential-owner refusal or refresh failure from a repeated usage rejection, and must not be described as proof of invalid refresh credentials. The account's existing authentication retry floor is preserved. All-account freshness and the final subscription claim remain incomplete for that identity.

Use the existing targeted account-recovery flow only after the operator explicitly selects this account for live reconnection. Re-resolve its current runtime identity, retain the existing same-account verification and auth-owner persistence, then obtain a real usage observation and verify its plan, independent timestamp, normalized windows and installed Widget presentation. Do not clear cooldowns, edit credential files, change priority/affinity/service tier, manufacture balances, or redeem reset credits. A replaced identity, changed deployment, new provider rejection or later retry floor invalidates the pending account-specific acceptance. This is the remaining gate in the existing monitor acceptance record, not a new tracker or an all-ten completion claim.

### Targeted reconnect recovery accepted - 2026-09-11

Session `01a08348-fdd5-76a1-8112-2351fbdd7ac9`. The account-specific no-observation gate above is superseded by the following repair and actual installed readback. The successful targeted OAuth exchange had persisted fresh credentials, but its final runtime replacement omitted credential metadata and could overwrite the persistence hook's already hydrated entry. The existing reconnect builder now populates storage and runtime metadata from the same exchange. Exact-account validation and credential ownership are unchanged.

Clean source `a24c007a8638239b394e39f358954f758503ee28`, immutable tag `ler-live-20260911-a24c007a`, produces canonical SHA-256 `2cbc9107d9b0c8dfba14859bb2861b9bc379b9051f26e0efedb4ddb354bf9e25` with Go 1.26.2 and `vcs.modified=false`. The old-source overlay reproduces both credential regressions; corrected targeted tests, seven affected packages and the Codex executor subset pass. Exact-binary monitor/restart and respawner/automatic-rollback staging pass with zero real-account requests. The earlier ordinary combined-suite xAI timing limitation remains disclosed; this narrow repair does not claim a new full-suite pass.

After fresh disk/hash/owner/health preflight, an interruption warning and action-time acknowledgment, the existing guarded deployer promoted that unchanged candidate. The retained rollback matches `4a1f4e3fc2832426c5a3734f82a07bdf4bf6f01640f9d07907f92c6948210d4d`. Independent post-owner verification confirms the new canonical process, exact candidate hash and HTTP 200. All runtime account indices, enabled states and priorities match the sanitized baseline; affinity remains enabled. Ephemeral affinity bindings and monitor identity retirement are normal runtime lifecycle state, not manually preserved or reset data.

The selected account became active after normal cold-start credential hydration. Real routed traffic supplied a new passive Pro observation at 00:13:51 UTC on September 12. Installed Widget 0.43's read-only quota snapshot at 00:14:30 UTC reports the matching current monitor identity, available/enabled state, no re-sign-in requirement, non-stale usage and 100 percent weekly remaining, with an independently captured 00:14:15 UTC observation. Widget's installed image is unchanged and its existing process remains running. No second OAuth exchange, manual monitoring intent, model test send, provider cooldown clear or reset action was used for this acceptance.

The main five-hour entitlement is still absent from this passive evidence; additional five-hour quota cannot substitute for it. Reset inventory remains independently unobserved at this readback. Preserve those unknowns until their existing owners obtain actual observations. The selected card was not exposed by the bounded native accessibility read, so this is installed-CLI runtime/data acceptance rather than screenshot or visual-layout proof. Configuration, Auto policies, Widget implementation, Desktop, SSH and App Servers are unchanged; the separately discussed new Widget UI remains deferred.
